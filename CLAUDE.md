...
**Asset-conflict flavours.** `proposedAssetFields.collisionReason` distinguishes two pathways the discovery sync can raise a hostname-collision conflict:
- `"untagged-collision"` — incoming Entra/AD device's hostname matches an existing asset that has no source tag (DHCP-discovered, manually created, etc.). Original behaviour.
- `"duplicate-registration"` — incoming hostname matches an existing asset already tagged by the same source with a *different* deviceId/objectGUID (Entra returned two distinct deviceIds for the same display name — re-enrol, re-image, dual-boot — or AD has two `objectGUID`s for what looks like the same physical computer). Accepting replaces the existing assetTag with the new deviceId; the prior deviceId is overwritten.

`proposedAssetFields.matchedVia` is `"exact"` or `"netbios"`. NetBIOS-truncation matching pairs an AD `cn`-derived 15-char hostname with the full Entra `displayName` and vice versa, so neither side missing the FQDN-aware rename causes a phantom split.
