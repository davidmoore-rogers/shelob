...
> **Multi-source asset model (Phase 2 cutover, firewall scope):** The FortiGate firewall asset itself is on the new `AssetSource` model — every discovered firewall (FMG-managed and standalone alike) gets a `sourceKind="fortigate-firewall"` row keyed on `externalId = serialNumber` with a rich source-shaped `observed` blob (serial, hostname, model, osVersion, mgmtIp, lat/long, managedBy: "fortimanager"|"fortigate"). Re-discovery still keys on `Asset.serialNumber` via the in-memory `findBySerial` index — the source row exists for the per-source detail view rather than for primary lookup, so the discovery hot path is unchanged. **Out of scope for this round:** managed FortiSwitch / FortiAP assets and DHCP-discovered endpoint hosts — both still write Asset rows directly via the legacy path; their dedicated source kinds (`fortiswitch`, `fortiap`, `fortigate-dhcp-host`) are sketched in CLAUDE.md but not yet cut over.
