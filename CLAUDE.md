...
- `POST   /device-icons`                         *(admin)* — `multipart/form-data` upload. Fields: `file` (required), `scope` ("type"|"model"|"manufacturer"), `key`. Re-uploading the same `(scope, key)` pair replaces the existing image atomically.
- `DELETE /device-icons/:id`                     *(admin)* — Remove an uploaded icon.
- `GET    /device-icons/:id/image`               *(auth)* — Serves raw bytes with the stored `Content-Type` and `Cache-Control: private, max-age=3600`. SVG responses additionally carry `X-Content-Type-Options: nosniff` + a strict CSP so direct-URL access is locked down. Topology endpoint embeds this URL in each node's `iconUrl` so Cytoscape's `background-image` renders the uploaded icon directly. Browser HTTP cache deduplicates fetches across re-renders.
