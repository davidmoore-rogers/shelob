...
│   │   ├── vendorTelemetryProfiles.ts # Per-vendor SNMP CPU/memory profile registry. Maps `manufacturer + os + model` regex → symbolic OID names; resolved through oidRegistry at probe time. Built-ins for Cisco / Juniper / Mikrotik / Fortinet FortiSwitch (matched before generic Fortinet via `model="FortiSwitch"`; memory uses bytes form via `fsSysMemUsage` @ 12356.106.4.1.3 + `fsSysMemCapacity` @ 12356.106.4.1.4 — NOT a percent like the FortiGate variant — both seeded into oidRegistry so the probe works without uploading FORTINET-FORTISWITCH-MIB. CPU OID not yet identified; falls through to HOST-RESOURCES-MIB) / Fortinet (SNMP path) / HP-Aruba / Dell.
