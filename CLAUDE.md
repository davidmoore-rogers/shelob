...
│       ├── table-sf.js
│       ├── mobile/                  # Mobile SPA bundle (loaded by mobile.html). router.js (hash routing), auth.js (login + TOTP screens), map-tab.js (Device Map tab — lazy-loads Leaflet/markercluster, plots health-coded firewall pins, opens a bottom sheet on tap; backs both #map and #site/<id>; site sheet's "View topology" button hands off to the in-app topology-tab via `topology/<siteId>` route — desktop link removed; renders a stick-figure-with-hardhat user-location pin via `navigator.geolocation.watchPosition` with browser-reported speed + haversine fallback gated on GPS accuracy, walking-animation CSS plays only when actually moving, auto-starts when `permissions.query({name:"geolocation"})` is `"granted"` else after first recenter tap, GPS watch torn down on hashchange away from #map / #site), topology-tab.js (mobile topology surface — lazy-loads cytoscape/dagre/cytoscape-dagre on first open, calls into shared topology-render.js, top-to-bottom dagre layout, no drag-to-reposition / no localStorage persistence / no screenshot / no fullscreen toggle, initial viewport zooms to the FortiGate or to a focus node when `?q=<term>` is in the URL, in-bar search wires `/topology/search` for endpoint→switch pivots, node tap opens a bottom sheet with monitor pill + endpoints list + "View asset" button; reached via `topology/<siteId>` detail route, fullbleed mode hides topbar slot + bottom navbar via `app.dataset.tab="__fullbleed"`), assets-tab.js (Assets tab — paged card feed with type filter chips, tap-through to asset detail), tabs.js (per-tab registry + Search renderer + shared snackbar helper), details.js (detail-route renderers — asset / subnet / block / site / topology; site delegates to map-tab, topology delegates to topology-tab), app.js (orchestrator: bootstraps auth, mounts the tab shell, dispatches tab and detail routes; detail specs may carry a `parentTab` to keep the corresponding navbar item highlighted; bootstraps a best-effort `screen.orientation.lock("portrait")` — Android Chrome / installed PWAs honor it, iOS Safari rejects silently and the CSS landscape lockout in mobile.css picks up the slack).
...
│   │   ├── fmgWorker.ts             # Per-integration single-consumer worker that serializes ALL traffic to a given FortiManager instance. Module-level `Map<integrationId, FmgWorker>`, lazily created on first `submit()`. Every code path that talks to FMG funnels through `getFmgWorker(integrationId).submit(label, task, signal)` so "FMG drops parallel API calls past 1-2 for the same API user" becomes an enforceable architectural invariant rather than convention. Exports `polaris_fmg_worker_queue_depth{integrationId}` and `polaris_fmg_worker_inflight{integrationId}` gauges so a slow FMG that's bottlenecking all of discovery + push + monitoring is visible in `/metrics`.
...
│   │   ├── queueService.ts          # Monitor work queue mode + pg-boss runtime. Two implementations: `cursor` (default — in-process worker pool inside runMonitorPass; fits small/medium fleets fine after the Step 4a split-tick fix) and `pgboss` (durable queue with per-cadence worker pools — recommended once the fleet crosses ~500 monitored assets). Active mode lives in `Setting.monitor.queueMode`; captured at boot via `initializeQueue()` and frozen as the running process's mode regardless of subsequent Setting writes (operator changes take effect on next restart). `startPgbossWorkers()` boots pg-boss (only when boot-time mode is `pgboss`), creates the four queues with `policy: "singleton"` + per-asset singletonKey for natural coalescing (duplicate `(assetId, cadence)` sends while a job is queued or active are absorbed silently; different assetIds run in parallel up to `localConcurrency`), and registers worker handlers that call back into the same `runFooFor()` functions the cursor pass uses. `publishMonitorJob(cadence, assetId, transport?)` is the publisher entry point; `monitorAssets.ts` calls it from the same two ticking loops it uses for cursor — only the dispatch differs. Worker counts default CPU-aware: each of the four queues (probe / fastFiltered / telemetry / systemInfo) gets `max(16, min(64, cpus().length × 4))` localConcurrency; env-overridable via `POLARIS_MONITOR_PROBE_WORKERS` / `POLARIS_MONITOR_FAST_WORKERS` / `POLARIS_MONITOR_HEAVY_WORKERS` (the heavy var sets both telemetry and systemInfo to the same value). Symmetric defaults because all four cadences are I/O-bound — earlier asymmetric defaults (probe `max(16, N×4)` / heavy `max(4, N)`) overprovisioned the cheapest cadence and underprovisioned the cadences most likely to wedge on a slow host; the 64-cap protects boxes with many cores from running the connection pool dry. `POLARIS_PGBOSS_POOL_SIZE` defaults to 20 (sized for the 32-default-workers / 64-cap headroom). The configured count is exposed as the `polaris_monitor_workers{queue}` gauge so operators can confirm restart-pickup of new core counts without reading logs. Naming convention `polaris-monitor-*`; future `polaris-discovery-*` queues should be added when discovery moves under pg-boss and worker tuning revisited together.
...
### FMG worker (per-integration single-consumer queue)

Every code path that talks to FortiManager funnels through a per-integration `FmgWorker` (`src/services/fmgWorker.ts`). The worker is a FIFO single-consumer queue keyed on `integrationId`; lazily created on first `submit()` and never torn down. The constraint it encodes — "FMG drops parallel API calls past 1-2 for the same API user" — is then enforceable in code review instead of an implicit serialization in one resolver. Cross-feature serialization comes for free: an operator clicking "Reserve IP" mid-discovery no longer races a mgmt-IP resolve against the reservation write — both go through the same queue, FIFO. Routed through the worker today: `rpc()` (covers ~all FMG calls including `fmgProxyRest`'s wrap), `resolveDeviceMgmtIp` (called from discovery + reservation push + quarantine push + monitoring's controller-IP cache miss + manual proxy/direct), `proxyQueryViaFortigate`, `testConnection` (when an integration id is in scope — pre-create test calls run direct since no other code can be talking to that FMG yet). Exported as `polaris_fmg_worker_queue_depth{integrationId}` and `polaris_fmg_worker_inflight{integrationId}` on `/metrics`.
...
Proxy mode (`useProxy=true`) ignores the warm cache — every per-device call funnels through `/sys/proxy/json` which is FMG-bound, so a cache offers no speedup. The same FMG worker still serializes those calls AND any concurrent reservation push / quarantine push / connection test against the same integration.
...
- `GET /metrics` — Prometheus text-format endpoint, gated by `METRICS_TOKEN` (auto-generated at first-run setup; clearing it reopens the endpoint and surfaces the `metrics_token_unset` watch-severity reason on the Maintenance tab — leaving `/metrics` open on a public deployment leaks fleet size, monitor health, monitor pass duration, and queue depth as recon data). Exposes default Node.js process / GC / event-loop metrics (un-prefixed so standard Grafana dashboards work) plus Polaris-specific gauges, histograms, and counters defined in `src/metrics.ts`. Polaris-specific metric names are prefixed `polaris_`. Currently exported:
  ...
  - `polaris_fmg_worker_queue_depth{integrationId}` / `polaris_fmg_worker_inflight{integrationId}` (gauges) — per-integration FortiManager worker (see `src/services/fmgWorker.ts`). Queue depth is the count of FMG-bound tasks waiting their turn (discovery resolves, reservation push, quarantine push, manual proxy, etc.); inflight is 1 when the worker is currently executing a task and 0 when idle. A persistently non-zero queue depth means FMG is the bottleneck for cross-feature traffic; correlate against `polaris_monitor_pass_duration_seconds` and discovery slow-run telemetry to confirm.
