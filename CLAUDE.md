...
│       ├── dashboard.js              # New home Dashboard — fetches /dashboard/summary and renders four cards: Monitor Alerts, Recently Reserved, Assets by Type (in-house SVG pie), Block Utilization. Re-ticks the duration labels on the Monitor Alerts card every 30s without re-fetching.
│       ├── blocks.js                  # IP Blocks tab content. Exposes `window.PolarisBlocks.init()` so `ipam.js` can drive lifecycle on tab activation; suppresses its own DOMContentLoaded auto-run when `window.__polarisIpamTabs=true`.
│       ├── subnets.js                 # Networks tab content. Exposes `window.PolarisSubnets.init()` + `window.PolarisSubnets.applyHashFilters()`; the latter reads `#tab=networks&block=<id>` / `&subnet=<id>` / `&focusReservation=<id>` and seeds the block filter + opens the IP panel BEFORE the first list load. Same auto-run-suppression contract as blocks.js.
│       ├── ipam.js                    # IPAM tabbed-page orchestrator. Reads `#tab=blocks|networks` from the hash, clones the matching `<template>` into `#ipam-mount`, and calls the tab module's exposed init(). Tab switch unmounts the other tab's DOM so the colliding IDs (filter-pagesize, pagination, pagination-top) stay unique. Uses history.replaceState on tab clicks to avoid back-button entries per click.
│       ├── assets.js
...
│   │       ├── dashboard.ts         # `/dashboard/summary` — one round-trip backing the home Dashboard's four cards. Pulls blockUtilization from utilizationService.getGlobalUtilization, recentReservations from utilizationService.getRecentManualReservations (sourceType=manual only), assetTypeCounts via prisma.asset.groupBy (excluding decommissioned/disabled), and monitorAlerts via prisma.asset.findMany filtered to monitored + status in (warning, down), ordered by monitorStatusChangedAt asc nulls-last, capped at 50.
...
│   │   ├── vendorTelemetryProfiles.ts # Per-vendor SNMP CPU/memory profile registry. Maps `manufacturer + os + model` regex → symbolic OID names; resolved through oidRegistry at probe time. Built-ins for Cisco / Juniper / Mikrotik / Fortinet FortiSwitch (matched before generic Fortinet via `model="FortiSwitch"`; CPU uses `fsSysCpuUsage` @ 12356.106.4.1.2 scalar percent; memory uses bytes form via `fsSysMemUsage` @ 12356.106.4.1.3 + `fsSysMemCapacity` @ 12356.106.4.1.4 — NOT a percent like the FortiGate variant — all three seeded into oidRegistry so the probe works without uploading FORTINET-FORTISWITCH-MIB) / Fortinet (SNMP path) / HP-Aruba / Dell.
...
Asset
  ...
  longitude       Float?
  fortinetTopology Json?           -- { role: "fortigate" | "fortiswitch" | "fortiap", controllerFortigate?, uplinkInterface?, parentSwitch?, parentPort?, parentVlan?, haMode?, haRole?, haPeerSerial? } — real connection graph from FMG/FortiGate discovery. `haMode` / `haRole` / `haPeerSerial` are populated only on firewall-role assets in an HA cluster (a-p / a-a) by the per-member Phase 3 fan-out; `haRole` is "primary" | "secondary", `haPeerSerial` points at the OTHER member's serial. Standalone firewalls omit the HA fields or carry `haMode: "standalone"`.
  acquiredAt      DateTime?
  ...
  telemetryTimeoutMs Int?         -- Per-asset telemetry collector timeout (ms). Range 1000..120000; null inherits from the resolved tier (default 10000ms). Applied to FortiOS REST + SNMP session timeouts inside collectTelemetry (CPU + memory + temperature) and the matching FortiOS sensor-info call. Distinct from probeTimeoutMs so the cheap response-time probe and the heavier telemetry scrape can be tuned independently — telemetry usually wants a longer ceiling because a busy SNMP agent can take seconds to answer a multi-OID walk.
  systemInfoTimeoutMs Int?        -- Per-asset interface / storage / LLDP collector timeout (ms). Range 1000..120000; null inherits from the resolved tier (default 10000ms). Applied to FortiOS REST + SNMP sessions inside collectSystemInfo + collectFastFiltered (the per-minute fast-cadence pin pull). Same independent-tuning rationale as telemetryTimeoutMs — the interface scrape on a 48-port FortiSwitch hits IF-MIB + ifXTable + LLDP-MIB + storage in one session and benefits from a separate budget.
...
AssetStorageSample              -- System tab per-mountpoint storage snapshot. SNMP only — FortiOS doesn't expose mountable storage and WinRM is not yet supported. Primary source is HOST-RESOURCES-MIB hrStorageTable (one row per fixed/removable disk). When that table is empty (devices whose SNMP agents don't implement HRM — FortiSwitches, some access points), collectSystemInfoSnmp consults the matched VendorTelemetryProfile.disk shape and synthesizes a single-row sample from the vendor's used/total scalars (e.g. FortiSwitch `fsSysDiskUsage` / `fsSysDiskCapacity`, mountPath "flash").
  id            UUID PK
  assetId       UUID FK → Asset (cascade delete)
  timestamp     DateTime        @default(now())
