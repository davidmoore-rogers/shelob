...
│   │   ├── queueService.ts          # Monitor work queue mode + pg-boss runtime. Two implementations: `cursor` (default — in-process worker pool inside runMonitorPass; fits small/medium fleets fine after the Step 4a split-tick fix) and `pgboss` (durable queue with per-cadence worker pools — recommended once the fleet crosses ~500 monitored assets). Active mode lives in `Setting.monitor.queueMode`; captured at boot via `initializeQueue()` and frozen as the running process's mode regardless of subsequent Setting writes (operator changes take effect on next restart). `startPgbossWorkers()` boots pg-boss (only when boot-time mode is `pgboss`), creates the four queues with `policy: "exclusive"` + per-asset singletonKey for natural coalescing, and registers worker handlers that call back into the same `runFooFor()` functions the cursor pass uses. `publishMonitorJob(cadence, assetId, transport?)` is the publisher entry point; `monitorAssets.ts` calls it from the same two ticking loops it uses for cursor — only the dispatch differs. Worker counts default CPU-aware: each of the four queues (probe / fastFiltered / telemetry / systemInfo) gets `max(8, cpus().length × 2)` localConcurrency; env-overridable via `POLARIS_MONITOR_PROBE_WORKERS` / `POLARIS_MONITOR_FAST_WORKERS` / `POLARIS_MONITOR_HEAVY_WORKERS` (the heavy var sets both telemetry and systemInfo to the same value). Symmetric defaults because all four cadences are I/O-bound — earlier asymmetric defaults (probe `max(16, N×4)` / heavy `max(4, N)`) overprovisioned the cheapest cadence and underprovisioned the cadences most likely to wedge on a slow host. The configured count is exposed as the `polaris_monitor_workers{queue}` gauge so operators can confirm restart-pickup of new core counts without reading logs. Naming convention `polaris-monitor-*`; future `polaris-discovery-*` queues should be added when discovery moves under pg-boss and worker tuning revisited together.
