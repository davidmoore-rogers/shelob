...
- `POST   /assets/:id/probe-now`                *(user-or-above)* — Run an immediate response-time probe AND a telemetry + system-info pull; returns `{ success, responseTimeMs, error? }` for the response-time probe (telemetry/system-info errors are swallowed and visible only via the next System tab refresh).
