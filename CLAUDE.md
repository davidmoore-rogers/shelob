...
**Per-class FortiSwitch / FortiAP direct polling:** The FMG/FortiGate integration's `config` JSON also accepts `fortiswitchMonitor` and `fortiapMonitor` blocks, each `{ enabled: boolean, snmpCredentialId: string | null }`. Edited from the **FortiSwitches** and **FortiAPs** subtabs of the Monitoring tab. When `enabled` is true and a credential is selected, every newly-discovered FortiSwitch/FortiAP gets stamped with `monitored=true`, `monitorType="snmp"`, `monitorCredentialId=<id>`, `discoveredByIntegrationId=<integration>`. Existing assets are *only* re-stamped when the operator hasn't changed the type — detected as `monitorType` is null OR (monitorType="snmp" AND monitorCredentialId matches the integration's choice). Anything else (icmp, winrm, an SNMP credential we didn't pick, etc.) counts as an operator override and is preserved. Both subtabs warn that managed FortiSwitches/FortiAPs in FortiLink mode usually keep their own management plane locked down — direct polling only works when SNMP has been explicitly enabled on the device itself, which is the operator's responsibility.
