...
- `src/services/agentInstallService.ts:startInstall` — async SSH path: SFTP upload binary + conf + installer script to /tmp, `sudo -n bash` the installer, transition `pending → uploading → enrolling`. Failure lands as `installStatus="failed" + installError + agent.install_failed` event. Windows path returns a clear "not yet supported" error (Phase 4b will add it).
- `src/services/agentInstallService.ts:startUninstall` — async SSH mirror: SFTP upload uninstall script, run it, on success hard-delete the ManagedAgent row + emit `agent.uninstalled`. Failure lands as `installStatus="uninstall_failed" + agent.uninstall_failed` warning.
