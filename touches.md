...
- `Asset.learnedLocation` for firewalls is set from `devSnmpLocation` when SNMP returned a value, regardless of any prior value (SNMP always wins over the hostname fallback). When SNMP returned nothing, the legacy `existingAsset.learnedLocation || hostname` fallback chain kicks in. This is the only writer for firewall `learnedLocation` — the projection layer deliberately leaves it null on the `fortigate-firewall` source (hostname's already on Asset.hostname). `Asset.location` (the operator-overridable field) is NEVER touched by discovery so manual operator edits stick.
